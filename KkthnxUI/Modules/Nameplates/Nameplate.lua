...
local Plates = CreateFrame("Frame", nil, UIParent)
Plates:SetScript("OnEvent", function(self, event, ...) self[event](self, ...) end)
...
local threatUpdate = function(self, elapsed)
	...
	if self.elapsed >= 0.2 then
		...
		if C["nameplate"].healthvalue == true then
			self.healthBar.percent:SetText(K.ShortValue(valueHealth).." - "..(string.format("%d%%", math.floor((valueHealth/maxHealth)*100))))
...
local updatePlate = function(self)
	...
	if g + b == 0 then
		-- Hostile unit
		newr, newg, newb = 0.78, 0.25, 0.25
		self.healthBar:SetStatusBarColor(0.78, 0.25, 0.25)
	elseif r + b == 0 then
		-- Friendly unit
	...
	elseif r + g == 0 then
		-- Friendly player
		newr, newg, newb = 75/255,  175/255, 76/255
		self.healthBar:SetStatusBarColor(75/255,  175/255, 76/255)
	elseif 2 - (r + g) < 0.05 and b == 0 then
		-- Neutral unit
		newr, newg, newb = 218/255, 197/255, 92/255
		self.healthBar:SetStatusBarColor(218/255, 197/255, 92/255)
	else
	...
	self.healthBar:SetHeight(C["nameplate"].height * UIParent:GetEffectiveScale())
	self.healthBar:SetWidth(C["nameplate"].width * UIParent:GetEffectiveScale())
	...
	self.castBar:SetHeight(C["nameplate"].height * UIParent:GetEffectiveScale())
	self.castBar:SetWidth(C["nameplate"].width * UIParent:GetEffectiveScale())
...
local fixCastbar = function(self)
	...
	self:SetHeight(C["nameplate"].height * UIParent:GetEffectiveScale())
	self:ClearAllPoints()
...
local createPlate = function(frame)
	...
	newNameRegion:SetFont(C["font"].nameplates_font, C["nameplate"].fontsize, C["font"].nameplates_font_style)
	newNameRegion:SetTextColor(1, 1, 1)
	frame.name = newNameRegion
 
	frame.level = levelTextRegion
	levelTextRegion:SetFont(C["font"].nameplates_font, C["nameplate"].fontsize, C["font"].nameplates_font_style)
	...
	castBar.time:SetFont(C["font"].nameplates_font, C["nameplate"].fontsize, C["font"].nameplates_font_style)
	castBar.time:SetPoint("RIGHT", castBar, "RIGHT", 0, 0.5)
	castBar.time:SetJustifyH("RIGHT")
 
	if C["nameplate"].showcastbarname == true then
		castBar.castName = castBar:CreateFontString(nil, "OVERLAY")
		castBar.castName:SetFont(C["font"].nameplates_font, C["nameplate"].fontsize, C["font"].nameplates_font_style)
...
if C["nameplate"].combat == true then
	...
	function Plates:PLAYER_REGEN_ENABLED()
		SetCVar("nameplateShowEnemies", 0)
	end

	Plates:RegisterEvent("PLAYER_REGEN_DISABLED")
	function Plates.PLAYER_REGEN_DISABLED()
