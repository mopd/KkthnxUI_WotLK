...
local MapShrink = function()
	...
	KkthnxUI.AddBorder(bgframe, 12, 0)
	...
	WorldMapDetailFrame:SetPoint(unpack(C["position"].worldmap))
...
local function CreateText(offset)
	...
	text:SetFont(C["media"].normal_font, 17)
...
if C["map"].group_icons then
	...
	function f:OnUpdate(icon, unit)
		...
		if string.find(unit, "raid", 1, true) then
			...
			icon:SetTexture(string.format("Interface\\AddOns\\KkthnxUI\\Media\\Worldmap\\Group%d", group))
		end
...
if C["map"].bg_map_stylization then
	...
	bm:SetScript("OnEvent", function(self, event, addon)
		...
		KkthnxUI.AddBorder(background, 10, 0)
		background:SetFrameLevel(0)
		background:SetPoint("TOPLEFT", -0, 0)
		background:SetPoint("BOTTOMRIGHT", -6, 4)
		background:SetBackdropBorderColor(K.Color.r, K.Color.g, K.Color.b)
