...
for i = 1, numf do
	...
	if i == 1 then
		...
		if C.unitframe.enable == true then
			f:SetPoint("BOTTOMLEFT", "PlayerFrame", "TOPLEFT", -3, 60)
		else
	...
	elseif i == 3 then
		...
		if C.unitframe.enable == true then
			f:SetPoint("BOTTOMRIGHT", "TargetFrame", "TOPRIGHT", 2, 278)
		else
			...
			if tslu > 0.5 then
				...
				for k, v in pairs(SQ) do
					if not SQ[k]["locked"] and SQ[k]["queue"] > 0 and SQ[k]["utime"] <= utime then
						...
						if C["combattext"].short_numbers == true then
							SQ[k]["queue"] = K.ShortValue(SQ[k]["queue"])
						end
				...
				if C["combattext"].healing then
					...
					if amount >= C["combattext"].heal_treshold then
						...
						if C["combattext"].show_overhealing and abs(overhealing) > 0 then
							if C["combattext"].short_numbers == true then
								amount = K.ShortValue(math.floor(amount-overhealing)).." ["..K.ShortValue(floor(overhealing)).."]"
							else
