...
local bind = CreateFrame( "Frame", "KkthnxUIHoverBind", UIParent )

SlashCmdList.MOUSEOVERBIND = function()
	if( InCombatLockdown() ) then print( L_BIND_COMBAT ) return end
	if not bind.loaded then
		...
		function bind:Listener( key )
			...
			if( key == "LSHIFT" or key == "RSHIFT" or key == "LCTRL" or key == "RCTRL" or key == "LALT" or key == "RALT" or key == "UNKNOWN" or key == "LeftButton" ) then return end
		...
		function bind:Deactivate( save )
			if( save ) then
				...
				LoadBindings( 2 )
				print( L_BIND_DISCARDBIND )
			end
			self.enabled = false
			self:HideFrame()
			self:UnregisterEvent( "PLAYER_REGEN_DISABLED" )
			StaticPopup_Hide( "KKTHNXUI_KEYBIND_MODE" )
		end
		
		StaticPopupDialogs["KKTHNXUI_KEYBIND_MODE"] = {
			text = L_BIND_INSTRUCT,
			button1 = L_BIND_SAVE,
			button2 = L_BIND_DISCARDBIND,
	...
	if not bind.enabled then
		bind:Activate()
		StaticPopup_Show( "KKTHNXUI_KEYBIND_MODE" )
	end
end
