...
frame:SetScript("OnEvent", function(self, _, ...)
	...
	if not InGroup() or InCombatLockdown() or not subEvent or not spellID or not sourceName then return end
	...
	if subEvent == "SPELL_CAST_SUCCESS" then
		...
		if C["Announcements"].Feasts and (spellID == 46887 or spellID == 43015 or spellID == 43478 or spellID == 34753 or spellID == 45279 or spellID == 43480) then
			SendChatMessage(format(L_ANNOUNCE_FP_PRE, sourceName, GetSpellLink(spellID)), K.CheckChat(true))
		-- Refreshment Table
		...
		elseif C["Announcements"].Portals and spellID == 698 or spellID == 29893 or spellID == 58887 then
			SendChatMessage(format(L_ANNOUNCE_FP_CLICK, sourceName, GetSpellLink(spellID)), K.CheckChat(true))
		-- Piccolo of the Flaming Fire
		elseif C["Announcements"].Toys and spellID == 18400 then
			SendChatMessage(format(L_ANNOUNCE_FP_USE, sourceName, GetSpellLink(spellID)), K.CheckChat(true))
		end
	elseif subEvent == "SPELL_SUMMON" then
		-- Repair Bots
		if C["Announcements"].Feasts and K.AnnounceBots[spellID] then
			SendChatMessage(format(L_ANNOUNCE_FP_PUT, sourceName, GetSpellLink(spellID)), K.CheckChat(true))
		end
	elseif subEvent == "SPELL_CREATE" then
		...
		elseif C["Announcements"].Portals and K.AnnouncePortals[spellID] then
			SendChatMessage(format(L_ANNOUNCE_FP_CAST, sourceName, GetSpellLink(spellID)), K.CheckChat(true))
		end
